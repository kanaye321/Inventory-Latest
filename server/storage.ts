...
export class MemStorage implements IStorage {
  ...
  // BitLocker keys operations
  private bitlockerKeys: any[] = [];

  async getBitlockerKeys(): Promise<any[]> {
    // Always use memory storage for MemStorage class
    console.log(`Using memory storage - found ${this.bitlockerKeys.length} BitLocker keys`);
    return this.bitlockerKeys;
  }

  async getBitlockerKey(id: number): Promise<any> {
    return this.bitlockerKeys.find(k => k.id === id);
  }

  async getBitlockerKeyBySerialNumber(serialNumber: string): Promise<any[]> {
    return this.bitlockerKeys.filter(k => k.serialNumber === serialNumber);
  }

  async getBitlockerKeyByIdentifier(identifier: string): Promise<any[]> {
    return this.bitlockerKeys.filter(k => k.identifier === identifier);
  }

  async createBitlockerKey(key: any): Promise<any> {
    try {
      if (this.databaseStorage) {
        console.log('Attempting to create BitLocker key in PostgreSQL...');
        const result = await this.databaseStorage.createBitlockerKey(key);
        console.log('✅ BitLocker key created successfully in PostgreSQL');
        return result;
      }
    } catch (error) {
      console.log('❌ PostgreSQL not available for BitLocker key creation, using memory storage fallback');
      console.log('Database error:', error.message);
    }

    // Memory storage fallback
    console.log('💾 Creating BitLocker key in memory storage...');
    ...
    this.bitlockerKeys.push(newKey);
    ...
    console.log('✅ BitLocker key created successfully in memory storage');
    return newKey;
  }

  async updateBitlockerKey(id: number, keyData: any): Promise<any> {
    const keyIndex = this.bitlockerKeys.findIndex(k => k.id === id);
    if (keyIndex === -1) return undefined;

    this.bitlockerKeys[keyIndex] = {
      ...this.bitlockerKeys[keyIndex],
      ...keyData,
      updatedAt: new Date()
    };

    return this.bitlockerKeys[keyIndex];
  }
  ...
  async deleteBitlockerKey(id: number): Promise<boolean> {
    ...
    this.bitlockerKeys.splice(keyIndex, 1);
    return true;
  }
