...
export async function initializeDatabase() {
  try {
    ...
    // Add missing columns to existing components table
    const componentsColumns = [
      'ALTER TABLE components ADD COLUMN IF NOT EXISTS type TEXT NOT NULL DEFAULT \'Unknown\'',
      'ALTER TABLE components ADD COLUMN IF NOT EXISTS status TEXT DEFAULT \'available\'',
      'ALTER TABLE components ADD COLUMN IF NOT EXISTS warranty_expiry TEXT',
      'ALTER TABLE components ADD COLUMN IF NOT EXISTS assigned_to TEXT',
      'ALTER TABLE components ADD COLUMN IF NOT EXISTS specifications TEXT'
    ];

    for (const columnSql of componentsColumns) {
      try {
        await db.execute(sql.raw(columnSql));
      } catch (error) {
        // Ignore errors for columns that already exist
        if (!error.message.includes('already exists')) {
          console.warn('Warning adding column to components:', error.message);
        }
      }
    }
...
async deleteVmInventoryItem(id: number): Promise<boolean> {
  ...
  async getJiraSettings(): Promise<any> {
    try {
      const result = await this.db.select().from(systemSettings).where(eq(systemSettings.id, 2)).limit(1);
      if (result[0] && result[0].siteName) {
        return JSON.parse(result[0].siteName);
      }
      return null;
    } catch (error) {
  ...
  async saveJiraSettings(settings: any): Promise<void> {
    try {
      ...
      const existing = await this.db.select().from(systemSettings).where(eq(systemSettings.id, 2)).limit(1);
      
      if (existing.length > 0) {
        // Update existing record
        await this.db.update(systemSettings)
          .set({ siteName: settingsJson })
          .where(eq(systemSettings.id, 2));
      } else {
        // Insert new record
        await this.db.insert(systemSettings).values({
          id: 2,
          siteName: settingsJson,
          siteUrl: 'jira-settings',
          companyName: 'JIRA Settings',
          companyAddress: '',
          companyEmail: '',
          defaultLanguage: 'en',
          defaultTimezone: 'UTC'
