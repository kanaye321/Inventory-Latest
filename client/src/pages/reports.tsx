...
export default function Reports() {
  ...
  const laptopStats = {
	...
	reservedlaptop: laptops.filter(laptop => laptop.status === 'Reserved').length
  ...
  const desktopStats = {
	...
	reserveddesktop: desktops.filter(desktop => desktop.status === 'Reserved').length
  };
      ...
      switch (reportType) {
        case 'device-summary':
          result = [
            { 
              ...
              pendingDeployment: laptopStats.pendingDeployment || 0,
              goodCondition: laptopStats.goodCondition || 0,
              badCondition: laptopStats.badCondition || 0,
			  reservedlaptop: laptopStats.reserved || 0
            },
            { 
              deviceType: 'Desktop', 
              ...
              pendingDeployment: desktopStats.pendingDeployment || 0,
              goodCondition: desktopStats.goodCondition || 0,
              badCondition: desktopStats.badCondition || 0,
			  reserveddesktop: desktopStats.reserved || 0
            }
          ];
          break;

        case 'department-summary':
          const deptKeys = Object.keys(departmentSummary || {});
          if (deptKeys.length > 0) {
            result = deptKeys.map(dept => ({
              department: dept,
        ...
        case 'user-device-analysis':
          if (usersWithOldSingleLaptop && usersWithOldSingleLaptop.length > 0) {
            result = usersWithOldSingleLaptop.map(user => {
              ...
              <Card className="backdrop-blur-sm bg-white/90 border-0 shadow-xl">
                ...
                <CardContent className="p-6">
                  <div className="space-y-4">
					...
					<div className="flex justify-between items-center p-4 bg-red-50 rounded-lg">
                      <span className="font-medium">Reserved Laptop:</span>
                      <Badge variant="outline" className="text-lg px-3 py-1 border-red-500 text-red-700">{laptopStats.reservedlaptop}</Badge>
                    </div>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card className="backdrop-blur-sm bg-white/90 border-0 shadow-xl">
                ...
                <CardContent className="p-6">
                  <div className="space-y-4">
					...
					<div className="flex justify-between items-center p-4 bg-red-50 rounded-lg">
                      <span className="font-medium">Reserved Desktop:</span>
                      <Badge variant="outline" className="text-lg px-3 py-1 border-red-500 text-red-700">{desktopStats.reserveddesktop}</Badge>
                    </div>
                  </div>
                </CardContent>
              </Card>
                ...
                <div>
                  ...
                  <Popover>
                    <PopoverTrigger asChild>
                      <Button
                        variant={"outline"}
                        className={format(dateRange.from, "PPP")}
                      >
